...
This tutorial follows Labs 1 through 5 from the MongoDB teach session.  Please review and complete lab 1 and 2 at a minimum, and enesure that you have an active and populated database to complete the following.
...
```{r environment}
mongolite::mongo(collection = 'Restaurants', db = 'Workshop', url = con) -> restaurantDB
...
## Specific queries
```{r smaller_query}
restaurantDB$find('{"borough": "Staten Island", "cuisine":"Chinese"}') -> staten_island_chinese
...
location = c(min(staten_island_chinese$long), min(staten_island_chinese$lat), max(staten_island_chinese$long), max(staten_island_chinese$lat)) %>% as.numeric()
